...
public:
	...
	Ray generateRay(float x, float y)
	{
		...
		float y_c = 2 * (1 - y / (height - 1)) - 1;
		Vec3 clip_point{ x_c, y_c, 1.0f };
		Vec3 d_camera = inverseProjectionMatrix.mulPointAndPerspectiveDivide(clip_point);
		Vec3 d = camera.mulVec(d_camera);
		d = d.normalize();
		return Ray(origin, d);
	}
